            ...
            {selectedArticle.content.split('\n\n').map((paragraph, idx) => {
              ...
              if (paragraph.startsWith('-')) {
                const listItems = paragraph.split('\n').map((item, i) => {
                  ...
                  if (rest.length > 0) {
                    return <li key={i} style={{ marginBottom: '12px', paddingLeft: isRtl ? '0' : '8px', paddingRight: isRtl ? '8px' : '0' }}><strong style={{ color: 'var(--primary-color)' }}>{boldPart.replace('**', '')}:</strong>{rest.join('**:')}</li>;
                  }
                  return <li key={i} style={{ marginBottom: '12px', paddingLeft: isRtl ? '0' : '8px', paddingRight: isRtl ? '8px' : '0' }}>{content}</li>;
                });
                return <ul key={idx} style={{ marginBottom: '24px', paddingLeft: isRtl ? '0' : '24px', paddingRight: isRtl ? '24px' : '0' }}>{listItems}</ul>;
              }
