  ...
  const fetchAdminData = async () => {
    ...
    try {
      ...
      const [usersRes, logsRes, rulesRes] = await Promise.all([
        ...
        fetch(`${API_URL}/api/shariah/rules`, { headers })
      ]);
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: '32px', alignItems: 'start' }}>
              <div className="glass-panel" style={{ position: 'sticky', top: '40px' }}>
                ...
                <form onSubmit={handleRuleSubmit}>
                  ...
                  <div style={{ marginBottom: '20px' }}>
                    <label style={{ fontSize: '12px', fontWeight: '700', textTransform: 'uppercase', color: 'var(--text-muted)', letterSpacing: '0.5px' }}>{t('admin.description')}</label>
                  ...
                  <div style={{ marginBottom: '32px' }}>
                    <label style={{ fontSize: '12px', fontWeight: '700', textTransform: 'uppercase', color: 'var(--text-muted)', letterSpacing: '0.5px' }}>{t('admin.source')}</label>
                ...
                {rules.map(rule => (
                  <div key={rule.rule_id} className="glass-panel" style={{ padding: '24px', borderLeft: !isRtl ? `4px solid ${rule.category.includes('prohibited') ? 'var(--danger)' : rule.category.includes('doubtful') ? '#F59E0B' : 'var(--primary-color)'}` : 'none', borderRight: isRtl ? `4px solid ${rule.category.includes('prohibited') ? 'var(--danger)' : rule.category.includes('doubtful') ? '#F59E0B' : 'var(--primary-color)'}` : 'none' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                      ...
                      <div style={{ display: 'flex', gap: '12px' }}>
                        <button onClick={() => setRuleForm({ ruleId: rule.rule_id, category: rule.category, description: rule.description, sourceReference: rule.source_reference || '' })} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontWeight: '600', fontSize: '13px' }}>{t('admin.editRule')}</button>
                        <button onClick={() => handleRuleDelete(rule.rule_id)} style={{ background: 'none', border: 'none', color: 'var(--danger)', cursor: 'pointer', fontWeight: '600', fontSize: '13px' }}>{t('admin.deleteRule')}</button>
                      </div>
                    </div>
