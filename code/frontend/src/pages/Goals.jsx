...
export default function Goals() {
  ...
  const API_URL = (import.meta.env.VITE_API_URL || 'http://localhost:8080') + '/api/goals';
  ...
  const handleDelete = async (goalId) => {
    ...
    try {
      ...
      setStatusMsg({ type: 'error', text: t('goals.error') });
    }
  };
        ...
        <div className="glass-panel goals-form-panel">
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ marginBottom: '20px' }}>
              <label style={{ fontSize: '13px', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--text-muted)' }}>{t('goals.goalType')}</label>
              <input 
                type="text" 
                name="goalType" 
            ...
            <div className="form-row-2" style={{ marginBottom: '20px' }}>
              ...
              <div>
                <label style={{ fontSize: '13px', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--text-muted)' }}>{t('goals.currentAmount')}</label>
                <input 
                  type="number" 
                  name="currentAmount" 
            ...
            <div style={{ marginBottom: '20px' }}>
              <label style={{ fontSize: '13px', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--text-muted)' }}>{t('goals.deadline')}</label>
              <input 
                type="date" 
                name="deadline" 
            ...
            {formData.goalId && (
              <div style={{ marginBottom: '28px' }}>
                <label style={{ fontSize: '13px', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--text-muted)' }}>{t('goals.status')}</label>
                <select 
                  name="status" 
          ...
          {loading ? (
            ...
            goals.map((goal) => {
              ...
              return (
                ...
                >
                  {/* Accent Line */}
                  <div style={{ position: 'absolute', top: 0, [isRtl ? 'right' : 'left']: 0, bottom: 0, width: '4px', background: badgeColor }}></div>
                  
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', paddingLeft: isRtl ? '0' : '12px', paddingRight: isRtl ? '12px' : '0' }}>
                    <div>
                      ...
                      {goal.deadline && (
                        <div style={{ fontSize: '13px', color: 'var(--text-muted)', marginTop: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                          ...
                          <strong style={{ color: 'var(--text-main)' }}>{new Date(goal.deadline).toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })}</strong>
                        </div>
                      )}
                    </div>
                    <span style={{ 
                  ...
                  <div style={{ margin: isRtl ? '32px 12px 24px 0' : '32px 0 24px 12px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '12px' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        <span style={{ fontSize: '13px', color: 'var(--text-muted)', fontWeight: '500' }}>{t('goals.progress')}</span>
                        <span style={{ fontSize: '28px', fontWeight: '800', color: badgeColor, lineHeight: '1' }}>
                          {progress}%
                        </span>
                      </div>
                      <div style={{ textAlign: isRtl ? 'left' : 'right', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    ...
                    <div style={{ height: '12px', background: 'var(--background)', borderRadius: '6px', position: 'relative', overflow: 'hidden', border: '1px solid var(--glass-border)' }}>
                      <div style={{ 
                        position: 'absolute',
                        top: 0,
                        bottom: 0,
