...
* **AI Shariah Advisor**: An intelligent chatbot powered by **Groq** and **RAG (Retrieval-Augmented Generation)** using **ChromaDB**. It searches uploaded Shariah law PDFs to provide accurate, context-aware answers to Islamic finance questions.
...
4. Run ChromaDB server on port 8000 via Docker.
5. Run `npm install` and `npm run dev` (or `node server.js`) in respective folders.
